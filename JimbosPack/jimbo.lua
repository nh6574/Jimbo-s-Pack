...
sadlad.calculate = function(self, card, context)
    ...
    if context.individual and context.cardarea == G.play then
        if context.other_card:is_face() then
            for i = 1, 50 do
                sendDebugMessage(currentscore)
            end
            return {
                card = card,
                chips = number_format(G.GAME.chips) * card.ability.extra.chips - number_format(G.GAME.chips),
                message = localize {
                    type = 'variable',
                    key = 'a_chips',
                    vars = {number_format(G.GAME.chips) * card.ability.extra.chips - number_format(G.GAME.chips)}
                },
            }
        end
    end
